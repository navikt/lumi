...
**File tools**: In this repo, manifests live under `nais/app/` (dev/prod).
...
In this repo, metrics are exposed at `/internal/prometheus`.

```kotlin
// See InternalRoutes.kt
```
...
1. Update `nais/app/dev.yaml` and `nais/app/prod.yaml`
2. Ensure operational endpoints exist (`/internal/isAlive`, `/internal/isReady`, `/internal/prometheus`)
3. Test locally with Docker / `./gradlew run`
4. Deploy to dev
5. Verify metrics/logs/traces in the NAIS observability stack
6. Deploy to prod
...
- Use environment-specific manifests (`nais/app/dev.yaml`, `nais/app/prod.yaml`)
...
- Skip health endpoints (`/internal/isAlive`, `/internal/isReady`)
