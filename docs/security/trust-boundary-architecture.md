...
|------------------|-------------------------------------------------------------------------------------|
| Autentisering    | Azure AD sidecar (Wonderwall) med autoLogin                                         |
| CSP              | Per-request nonce + SHA256-hash, `object-src 'none'`, `frame-ancestors 'none'`      |
| CSRF             | Origin/Referer-validering for non-GET metoder                                       |
| SRI              | SHA384-hashes for bundlede JS/CSS-assets                                            |
...
| Token-validering    | Texas sidecar introspection (ikke egenmontert JWT)                           |
...
| Kontroll         | Implementasjon                                                                   |
|------------------|----------------------------------------------------------------------------------|
| Token-validering | Texas introspection per request (TokenX eller AzureAD)                           |
...
| PII-redaksjon    | 13 mønstertyper, redaksjon ved lagring + lesing                                  |
...
| Cache        | Valkey med TTL (1t for team, 5min for tomt, 30s for feil) |
